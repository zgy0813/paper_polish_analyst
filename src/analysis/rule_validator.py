...
class RuleValidator:
    ...
    def validate_rules(self, official_rules: List[Dict], 
                      empirical_rules: List[Dict] = None) -> Dict:
        """
        验证规则一致性
        ...
        try:
            ...
            # 验证官方规则
            official_validation = self._validate_official_rules(official_rules, journal_texts)
            
            # 验证经验规则（如果提供）
            empirical_validation = {}
            if empirical_rules:
                empirical_validation = self._validate_empirical_rules(empirical_rules, journal_texts)
            
            # 检测规则冲突
            conflicts = self._detect_rule_conflicts(official_rules, empirical_rules or [])
    ...
    def _validate_official_rules(self, official_rules: List[Dict], 
                                journal_texts: List[str]) -> Dict:
        """验证官方规则"""
        validation_results = {
    ...
    def _validate_empirical_rules(self, empirical_rules: List[Dict], 
                                 journal_texts: List[str]) -> Dict:
        """验证经验规则"""
        validation_results = {
    ...
    def _check_rule_compliance(self, rule: Dict, journal_texts: List[str]) -> float:
        ...
        try:
            ...
            if 'examine' in description and 'investigate' in description:
                return self._check_vocabulary_preference(journal_texts, 'examine', 'investigate')
    ...
    def _check_vocabulary_preference(self, texts: List[str], 
                                   preferred: str, alternative: str) -> float:
        """检查词汇偏好"""
        preferred_count = 0
        alternative_count = 0
    ...
    def _detect_rule_conflicts(self, official_rules: List[Dict], 
    ...
    def _generate_validation_summary(self, official_validation: Dict, 
        ...
        return {
            ...
            'high_consistency_official': len(official_validation.get('high_consistency_rules', [])),
            'low_consistency_official': len(official_validation.get('low_consistency_rules', [])),
            'high_consistency_empirical': len(empirical_validation.get('high_consistency_rules', [])),
            'low_consistency_empirical': len(empirical_validation.get('low_consistency_rules', [])),
    ...
    def _calculate_overall_consistency(self, official_validation: Dict, 
                                     empirical_validation: Dict) -> float:
        """计算整体一致性分数"""
...
def main():
    ...
    if 'error' not in result:
        ...
        print(f"官方规则数: {result.get('official_rules_validation', {}).get('total_rules', 0)}")
    else:
