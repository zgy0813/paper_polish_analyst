...
class QualityScorer:
    ...
    def _calculate_style_match_score(self, paper_text: str, style_guide: Dict) -> Dict:
        ...
        try:
            ...
            # 根据规则类型调整权重
            core_rules = [r for r in rule_details if r['rule_type'] == 'core' and r['matched']]
            optional_rules = [r for r in rule_details if r['rule_type'] == 'optional' and r['matched']]
            ...
            weighted_score = (
                ...
                len(optional_rules) * 1.0  # 可选规则权重1.0
                 ...
                 len([r for r in rules if r.get('rule_type') == 'optional']) * 1.0) * 100
            ...
            return {
                ...
                'weaknesses': [r['description'] for r in rule_details if not r['matched'] and r['rule_type'] == 'core'][:3]
            }
    ...
    def _check_rule_match(self, paper_text: str, rule: Dict) -> bool:
        ...
        try:
            ...
            if 'examine' in description and 'investigate' in description:
                ...
                investigate_count = len(re.findall(r'\binvestigate\b', paper_text.lower()))
                return examine_count > investigate_count
            
            elif 'passive' in description or 'voice' in description:
                # 检查被动语态使用
                passive_ratio = self.nlp_utils.analyze_academic_expression(paper_text).get('passive_voice_ratio', 0)
    ...
    def _calculate_academic_standard_score(self, paper_text: str) -> Dict:
        ...
        try:
            ...
            if not self.client or not self.prompts:
                ...
                # 基于NLP指标计算分数
                passive_score = min(nlp_analysis.get('passive_voice_ratio', 0) * 100, 30)
                first_person_score = max(0, 20 - nlp_analysis.get('first_person_usage', {}).get('first_person_ratio', 0) * 100)
            ...
            logger.info(f"质量评估AI响应参数: model={response.model}, usage={response.usage}, finish_reason={response.choices[0].finish_reason}")
            ...
            # 提取学术规范性评分
            academic_score = gpt_result.get('assessment', {}).get('academic_standard', {})
    ...
    def _generate_detailed_analysis(self, paper_text: str, style_score: Dict, 
                                   academic_score: Dict, readability_score: Dict) -> Dict:
        """
        生成详细分析报告
        ...
        try:
            sentence_count = len(self.nlp_utils.analyze_sentence_structure(paper_text).get('total_sentences', 1))
        except:
        ...
        return {
            ...
            'readability_analysis': {
                ...
                'sentence_complexity': readability_score.get('sentence_length_score', 0),
                'vocabulary_diversity': readability_score.get('vocabulary_richness_score', 0),
                'transition_usage': readability_score.get('transition_score', 0)
            }
        }
    
    def _generate_recommendations(self, style_score: Dict, academic_score: Dict, 
                                 readability_score: Dict) -> List[str]:
        """
        生成改进建议
    ...
    def compare_scores(self, before_scores: Dict, after_scores: Dict) -> Dict:
        ...
        try:
            comparison = {
                'overall_improvement': after_scores.get('overall_score', 0) - before_scores.get('overall_score', 0),
                'style_improvement': after_scores.get('style_match', {}).get('score', 0) - before_scores.get('style_match', {}).get('score', 0),
                'academic_improvement': after_scores.get('academic_standard', {}).get('score', 0) - before_scores.get('academic_standard', {}).get('score', 0),
                'readability_improvement': after_scores.get('readability', {}).get('score', 0) - before_scores.get('readability', {}).get('score', 0),
                'before_scores': before_scores,
                'after_scores': after_scores
            }
            
            # 计算改进百分比
            for key in ['overall_improvement', 'style_improvement', 'academic_improvement', 'readability_improvement']:
                before_key = key.replace('_improvement', '')
                before_value = before_scores.get(before_key, {}).get('score', 0) if isinstance(before_scores.get(before_key), dict) else before_scores.get(before_key, 0)
