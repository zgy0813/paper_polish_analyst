...
class StyleGuideGenerator:
    ...
    def _categorize_rule(self, description: str) -> str:
        ...
        if any(word in description_lower for word in ['sentence', 'length', 'structure', 'compound']):
            ...
            return '段落组织'
        elif any(word in description_lower for word in ['transition', 'coherence', 'connect', 'flow', '衔接', '连接', '过渡']):
            return '段落衔接'
        ...
        for rule in rules:
            if 'examples' in rule and isinstance(rule['examples'], list):
                ...
                if '→' in evidence or 'to' in evidence:
                    # 简单的示例提取
                    examples.append({
                        'before': evidence.split('→')[0].strip() if '→' in evidence else evidence,
                        'after': evidence.split('→')[1].strip() if '→' in evidence else evidence,
    ...
    def _calculate_quality_metrics(self, rules: List[Dict]) -> Dict:
        ...
        return {
            ...
            'high_consistency_rules': len([r for r in rules if r.get('consistency_rate', 0) > 0.8]),
    ...
    def _generate_markdown_guide(self) -> str:
        ...
        md_content.append("")
        md_content.append(f"**生成时间**: {self.style_guide.get('generation_date', '')}")
        md_content.append(f"**分析论文数**: {self.style_guide.get('total_papers_analyzed', 0)}")
        md_content.append(f"**规则总数**: {self.style_guide.get('rule_summary', {}).get('total_rules', 0)}")
        md_content.append("")
        ...
        md_content.append("")
        md_content.append(f"- **核心规则**: {summary.get('core_rules', 0)} 条 (80%+论文遵循)")
        md_content.append(f"- **可选规则**: {summary.get('optional_rules', 0)} 条 (50%-80%论文遵循)")
        md_content.append(f"- **建议规则**: {summary.get('suggested_rules', 0)} 条 (遵循率较低)")
        md_content.append("")
        ...
        for rule_type, guideline in guidelines.items():
            md_content.append(f"- **{rule_type.replace('_', ' ').title()}**: {guideline}")
        md_content.append("")
        ...
        if quality_metrics:
            ...
            md_content.append("")
            md_content.append(f"- **平均遵循率**: {quality_metrics.get('avg_frequency', 0):.1%}")
            md_content.append(f"- **平均一致性**: {quality_metrics.get('avg_consistency', 0):.1%}")
            md_content.append(f"- **高一致性规则数**: {quality_metrics.get('high_consistency_rules', 0)}")
            md_content.append(f"- **覆盖度评分**: {quality_metrics.get('coverage_score', 0):.2f}")
            md_content.append(f"- **可靠性评分**: {quality_metrics.get('reliability_score', 0):.2f}")
            md_content.append("")
    ...
    def _save_guide_files(self):
        ...
        try:
            ...
            logger.info(f"风格指南已保存到 {Config.STYLE_GUIDE_JSON} 和 {Config.STYLE_GUIDE_MD}")
    ...
    def generate_hybrid_guide(self, official_rules: List[Dict] = None, 
        ...
        Returns:
            混合风格指南
        """
        logger.info("Starting hybrid style guide generation")
        ...
        try:
            ...
            logger.info(f"Hybrid style guide generation completed with {len(resolved_rules)} rules")
            return hybrid_guide
    ...
    def _generate_rule_summary(self, rules: List[Dict]) -> Dict:
        ...
        empirical_count = len([r for r in rules if r.get('source') == 'empirical_analysis'])
    ...
    def _calculate_hybrid_quality_metrics(self, rules: List[Dict]) -> Dict:
        ...
        return {
            ...
            'high_consistency_rules': len([r for r in rules if r.get('frequency', 0) >= 0.8]),
    ...
    def _generate_markdown_content(self, style_guide: Dict) -> str:
        ...
        if 'official_rules_count' in style_guide:
            content.append(f"**Official Rules**: {style_guide.get('official_rules_count', 0)}")
        if 'empirical_rules_count' in style_guide:
            content.append(f"**Empirical Rules**: {style_guide.get('empirical_rules_count', 0)}")
            ...
            for category, category_rules in categories.items():
                ...
                for rule in category_rules:
                    ...
                    if rule.get('source'):
                        ...
                        source = source_translation.get(rule.get('source', ''), rule.get('source', ''))
                        content.append(f"**Source**: {source}")
                    if rule.get('frequency'):
                        content.append(f"**Adherence Rate**: {rule.get('frequency', 0):.1%}")
                        ...
                        for example in rule.get('examples', []):
                            if isinstance(example, dict):
                                if 'correct' in example and 'incorrect' in example:
                                    content.append(f"- ✅ **Correct**: {example['correct']}")
                                    content.append(f"- ❌ **Incorrect**: {example['incorrect']}")
                                    if 'explanation' in example:
                                        content.append(f"  - **Explanation**: {example['explanation']}")
        ...
        'rules': [
            {
                ...
                'frequency': 0.78,
                'examples': [
                    {'before': 'This study investigates...', 'after': 'This study examines...'}
                ]
            }
        ],
