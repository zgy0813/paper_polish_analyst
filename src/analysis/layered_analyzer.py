...
class LayeredAnalyzer:
    ...
    def analyze_individual_paper(self, paper_id: str, paper_text: str) -> Dict:
        ...
        try:
            ...
            prompt = self.prompts.format_prompt(
                self.prompts.get_individual_analysis_prompt(),
    ...
    def analyze_all_individual_papers(self, max_papers: int = None, resume: bool = False) -> Dict:
        """
        分析所有单个PDF文件（独立于批量分析）
        ...
        try:
            ...
            logger.info(f"单个文件分析完成: 成功 {self.analysis_progress['completed_files']} 个, 失败 {self.analysis_progress['failed_files']} 个")
    ...
    def _filter_unanalyzed_files(self, extracted_files: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
        """
        过滤掉已经分析过的文件
        ...
        logger.info(f"过滤后需要分析的文件数量: {len(unanalyzed_files)}/{len(extracted_files)}")
    ...
    def _generate_analysis_summary(self, results: List[Dict]) -> Dict:
        ...
        Returns:
            分析摘要
        """
        successful_results = [r for r in results if 'error' not in r and 'parse_error' not in r]
        ...
        summary = {
            ...
            'failed_papers': [r['paper_id'] for r in failed_results],
    ...
    def analyze_batch(self, batch_id: str, individual_reports: List[Dict]) -> Dict:
        ...
        try:
            ...
            batch_input = {
                ...
                'individual_analyses': individual_reports
            }
            ...
            prompt = self.prompts.format_prompt(
                ...
                individual_analyses=json.dumps(individual_reports, ensure_ascii=False, indent=2)
            )
            ...
            batch_summary['input_papers'] = [report['paper_id'] for report in individual_reports]
    ...
    def integrate_global_style(self, batch_summaries: List[Dict]) -> Dict:
        ...
        try:
            ...
            # 使用GPT-4进行全局整合
            prompt = self.prompts.format_prompt(
                self.prompts.get_global_integration_prompt(),
                batch_summaries=json.dumps(batch_summaries, ensure_ascii=False, indent=2)
            )
            
            logger.info(f"全局整合Prompt长度: {len(prompt)} 字符")
            
            # 使用新的AI调用方法
            response_text = self._call_ai_api(prompt, 'global_integration')
    ...
    def _call_ai_api(self, prompt: str, task_type: str = 'default') -> str:
        ...
        try:
            ...
            max_tokens = min(
                self.max_tokens_config.get(task_type, 15000),
            ...
            logger.info(f"AI调用成功 - 任务类型: {task_type}, 温度: {temperature}, 最大tokens: {max_tokens}")
    ...
    def _get_system_message(self, task_type: str) -> str:
        ...
        messages = {
            ...
            'rule_generation': "You are a writing rule expert. Generate clear, actionable writing rules based on patterns. Respond with valid JSON only.",
            'example_extraction': "You are an example finder. Extract relevant, illustrative examples from text. Respond with valid JSON only.",
            'quality_assessment': "You are a quality evaluator. Assess writing quality objectively and consistently. Respond with valid JSON only.",
            'consistency_check': "You are a format validator. Ensure strict adherence to JSON format requirements. Respond with valid JSON only."
        }
...
def main():
    ...
    if texts:
        ...
        result = analyzer.analyze_individual_paper(f"test_{paper_id}", paper_text[:2000])  # 限制长度用于测试
