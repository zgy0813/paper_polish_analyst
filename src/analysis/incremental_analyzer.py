...
class IncrementalAnalyzer:
    ...
    def _create_batches(
        self, texts: List[Tuple[str, str]], batch_size: int
    ...
    def _analyze_batch(self, batch_id: str, batch_texts: List[Tuple[str, str]]) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"批次 {batch_id}: {skipped_count} 篇已分析，{need_analysis_count} 篇需要分析"
            )
            ...
            for paper_id, paper_text in batch_texts:
                if existing_reports[paper_id]:
                    ...
                    if existing_report:
                        ...
                        report = self.layered_analyzer.analyze_individual_paper(
                            paper_id, paper_text
                        )
            ...
            batch_summary = self.layered_analyzer.analyze_batch(
                batch_id, individual_reports
            )
            ...
            batch_result = {
                ...
                "individual_reports": [
                    report.get("paper_id") for report in individual_reports
                ],
            ...
            logger.info(
                f"完成批次 {batch_id}: {len(batch_texts)} 篇论文 "
                f"(跳过 {skipped_count} 篇individual_reports，分析 {need_analysis_count} 篇，"
                f"重新生成batch_summary)，耗时 {duration:.2f} 秒"
            )
    ...
    def _should_stop_analysis(
        self, current_batch: Dict, all_batches: List[Dict]
    ) -> bool:
        """
        基于规则多样性的停止条件判断
        ...
        # 1. 检查最小批次数
        if len(all_batches) < Config.MIN_BATCHES_FOR_DIVERSITY:
            logger.info(
                f"当前批次数 {len(all_batches)} < 最小要求 {Config.MIN_BATCHES_FOR_DIVERSITY}"
            )
            return False
        ...
        if len(all_batches) >= Config.MIN_BATCHES_FOR_DIVERSITY:
            ...
            logger.info(f"规则多样性: {diversity:.3f}")

            if diversity >= Config.RULE_DIVERSITY_THRESHOLD:
                logger.info(
                    f"规则多样性达到阈值 {Config.RULE_DIVERSITY_THRESHOLD}，停止分析"
                )
                return True

        # 3. 检查最近批次是否还有新规则补充
        if len(all_batches) >= 5:
            recent_batches = all_batches[-3:]  # 最近3个批次
            new_rules_count = self._count_new_rules(recent_batches)
            logger.info(f"最近3个批次新增规则: {new_rules_count}")

            if new_rules_count < 2:  # 最近3个批次新增规则少于2个
                logger.info("最近批次规则补充较少，停止分析")
                return True

        # 4. 检查是否达到最大批次数
        if len(all_batches) >= Config.MAX_BATCHES_FOR_DIVERSITY:
            logger.info(f"达到最大批次数 {Config.MAX_BATCHES_FOR_DIVERSITY}")
            return True
    ...
    def _calculate_rule_diversity(self, batch_summaries: List[Dict]) -> float:
        """
        计算规则丰富度而非重复度（基于并集思维）
        ...
        try:
            ...
            for batch in batch_summaries:
                if (
                    "batch_summary" in batch
                    and "preliminary_rules" in batch["batch_summary"]
                ):
                    rules = batch["batch_summary"]["preliminary_rules"]
                    for rule in rules:
            ...
            # 计算规则丰富度：唯一规则数量 / 期望的最小规则数量
            unique_rules = set(all_rules)
            expected_min_rules = len(batch_summaries) * 4  # 期望每个批次至少4条规则

            # 丰富度 = min(唯一规则数 / 期望最小规则数, 1.0)
            richness_ratio = min(len(unique_rules) / expected_min_rules, 1.0)

            logger.info(
                f"规则统计: 总规则{len(all_rules)}条, 唯一规则{len(unique_rules)}条, 期望最小{expected_min_rules}条"
            )

            return float(richness_ratio)

        except Exception as e:
            logger.error(f"计算规则丰富度失败: {str(e)}")
            return 0.0

    def _count_new_rules(self, recent_batches: List[Dict]) -> int:
        """
        统计最近批次新增的规则数量

        Args:
            recent_batches: 最近几个批次的结果
                ...
                if (
                    ...
                    for rule in rules:
                        if isinstance(rule, dict) and "description" in rule:
                            ...
                            current_batch_rules.append(rule_desc)

                            # 检查是否是新规则
                            if rule_desc not in all_rules:
                                new_rules_count += 1

                    all_rules.extend(current_batch_rules)

            return new_rules_count

        except Exception as e:
            logger.error(f"统计新规则数量失败: {str(e)}")
            return 0

    def _track_rule_evolution(self, batch_summaries: List[Dict]) -> Dict:
        """
        追踪规则演进过程
        ...
        try:
            ...
            for i, batch in enumerate(batch_summaries):
                ...
                if (
                    ...
                    for rule in rules:
                        if isinstance(rule, dict) and "description" in rule:
                            batch_rules.append(rule["description"])

                # 计算新规则数量
                new_rules = [rule for rule in batch_rules if rule not in all_rules]
                all_rules.extend(batch_rules)

                evolution[batch_id] = {
                    "rules_count": len(batch_rules),
                    "new_rules_count": len(new_rules),
                    "description": f"批次{i+1}：{len(batch_rules)}条规则，{len(new_rules)}条新规则",
                }

            return evolution

        except Exception as e:
            logger.error(f"追踪规则演进失败: {str(e)}")
            return {}
    ...
    def _generate_final_style_guide(self, batch_results: List[Dict]) -> Dict:
        ...
        try:
            ...
            # 使用分层分析器生成最终风格指南
            final_guide = self.layered_analyzer.integrate_global_style(
                successful_summaries
            )
