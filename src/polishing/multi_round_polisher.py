...
class MultiRoundPolisher:
    ...
    def polish_paper(self, paper_text: str, interactive: bool = True) -> Dict:
        ...
        try:
            ...
            if interactive:
                ...
                # 非交互式润色（一次性处理）
                result = self._batch_polishing()
            
            # 计算润色后评分
            after_scores = self.quality_scorer.score_paper(self.current_text)
            
            # 比较评分
            score_comparison = self.quality_scorer.compare_scores(before_scores, after_scores)
    ...
    def _interactive_polishing(self) -> Dict:
        ...
        for round_info in rounds:
            ...
            # 显示修改建议并获取用户选择
            user_choices = self._present_modifications_to_user(round_info, modifications)
    ...
    def _generate_round_modifications(self, round_info: Dict) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"润色轮次{round_info['round']}AI响应参数: model={response.model}, usage={response.usage}, finish_reason={response.choices[0].finish_reason}")
            logger.info(f"润色轮次{round_info['round']}AI响应内容长度: {len(response_content)} 字符")
            logger.info(f"润色轮次{round_info['round']}AI完整响应内容: {response_content}")
    ...
    def _get_relevant_rules(self, focus: str) -> List[Dict]:
        ...
        if focus == 'sentence_structure':
            ...
            relevant_categories = ['Paragraph Organization', 'Coherence', 'Transitions', '连接词', '段落衔接']
        else:
            return rules
    ...
    def _present_modifications_to_user(self, round_info: Dict, modifications: List[Dict]) -> Dict:
        """
        向用户展示修改建议并获取选择
    ...
    def _apply_user_choices(self, round_info: Dict, modifications: List[Dict], user_choices: Dict):
        """
        应用用户选择的修改
        ...
        for modification in accepted_modifications:
            ...
            if original_text in self.current_text:
                self.current_text = self.current_text.replace(original_text, modified_text, 1)
                logger.info(f"应用修改: {original_text[:50]}... → {modified_text[:50]}...")
    ...
    def _generate_polishing_summary(self) -> Dict:
        ...
        Returns:
            润色摘要
        """
        total_modifications = sum(round_info.get('modifications_applied', 0) 
    ...
    def save_polished_result(self, result: Dict, output_path: str):
        ...
        try:
            ...
            # 保存详细结果（JSON）
            json_path = output_path.replace('.txt', '_result.json').replace('.md', '_result.json')
...
def main():
    ...
    print(f"总分改进: {result.get('score_comparison', {}).get('overall_improvement', 0):.1f}")
