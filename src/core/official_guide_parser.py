...
class OfficialGuideParser:
    ...
    def __init__(self):
        ...
        # 缓存文件路径
        self.cache_file = Path("data/official_rules_cache.json")
    ...
    def parse_official_guide(self, pdf_path: str, force_refresh: bool = False) -> Dict:
        ...
        try:
            ...
            if not force_refresh and self.cache_file.exists():
                ...
                if cached_result:
                    logger.info(f"从缓存加载官方规则，共 {cached_result.get('total_rules', 0)} 条规则")
            ...
            text, metadata = self.pdf_extractor.extract_text_from_pdf(pdf_file)
            
            if not text or not metadata.get('extraction_success'):
    ...
    def _extract_rules_with_ai(self, text: str) -> List[Dict]:
        ...
        try:
            ...
            prompt = self.prompts.format_prompt(
                self.prompts.get_official_guide_parsing_prompt(),
            ...
            logger.info(f"官方指南解析AI响应参数: model={response.model}, usage={response.usage}, finish_reason={response.choices[0].finish_reason}")
    ...
    def save_official_rules(self, rules_data: Dict, output_path: str = None):
        ...
        """
        if not output_path:
            output_path = "data/official_rules.json"
