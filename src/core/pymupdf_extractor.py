...
class PyMuPDFExtractor:
    ...
    def extract_all_pdfs(self) -> Dict[str, Any]:
        ...
        for pdf_file in pdf_files:
            try:
                ...
                if result['success']:
                    ...
                    logger.info(f"成功提取: {pdf_file.name} ({result.get('character_count', 0)} 字符)")
                else:
                    results['failed_extractions'] += 1
                    logger.error(f"提取失败: {pdf_file.name} - {result.get('error', '未知错误')}")
        ...
        logger.info(f"提取完成: 成功 {results['successful_extractions']}, 失败 {results['failed_extractions']}")
        return results
    ...
    def extract_single_pdf(self, pdf_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            # 检查整体是否为双栏布局
            two_column_pages = sum(1 for page in layout_info['pages'].values() if page.get('is_two_column', False))
    ...
    def _reorganize_page_text(self, text_blocks: List[Dict], layout: Dict) -> str:
        ...
        if layout.get('is_two_column', False):
            # 双栏布局：按列重组
            page_center = sum(b['bbox'][0] for b in text_blocks) / len(text_blocks) + sum(b['bbox'][2] for b in text_blocks) / len(text_blocks)
            page_center /= 2
